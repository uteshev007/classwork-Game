...
//----------------------------------------------------
// Функции работы с манипулятором типа "мышь"
//

// ассинхронная (не блокирующая) проверка нажатия левой кнопки мыши
bool isLButtonDown() {
  SHORT state = GetAsyncKeyState(VK_LBUTTON);
  return (state & 0x8000) != 0;
}

// ассинхронная (не блокирующая) проверка нажатия правой кнопки мыши
bool isRButtonDown() {
  SHORT state = GetAsyncKeyState(VK_RBUTTON);
  return (state & 0x8000) != 0;
}

// ассинхронная (не блокирующая) проверка нажатия средней кнопки мыши
bool isMButtonDown() {
  SHORT state = GetAsyncKeyState(VK_MBUTTON);
  return (state & 0x8000) != 0;
}

// получить текущие координаты указателя мыши в пикселах экрана
...
// получить текущие координаты указателя мыши в консольных координатах
//   относительно видимой области консольного окна
...
// получить текущие координаты указателя мыши в консольных координатах
//   относительно видимой области консольного окна
// возвращает координаты в раздельных переменных для строки и столбца
